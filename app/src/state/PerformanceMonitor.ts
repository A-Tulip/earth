...
export class AdaptiveDegrader {
  ...
  feed(fps: number, avgFrameMs: number): void {
    this.sampleListeners.forEach((cb) => { try { cb(fps, avgFrameMs); } catch (e) { console.warn('[EmptyCatch] state/PerformanceMonitor.ts:244', e instanceof Error ? e.message : String(e)); } });
    // 入窗
  ...
  private emit(t: TierTransition): void {
    this.listeners.forEach((cb) => { try { cb(t); } catch (e) { console.warn('[EmptyCatch] state/PerformanceMonitor.ts:303', e instanceof Error ? e.message : String(e)); } });
  }
}
