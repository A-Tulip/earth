...
export function CesiumCanvas({ onReady }: CesiumCanvasProps) {
  ...
  useEffect(() => {
    ...
    try {
      const watchTiles = () => {
        ...
        for (let i = 0; i < layers.length; i++) {
          ...
          } catch (e) { console.warn('[EmptyCatch] cesium/CesiumCanvas.tsx:119', e instanceof Error ? e.message : String(e)); }
        }
      };
      watchTiles();
      // 后续 setBasemap 动态新增的 layer 也一并监听
      viewer.imageryLayers.layerAdded.addEventListener(() => watchTiles());
    } catch (e) { console.warn('[EmptyCatch] cesium/CesiumCanvas.tsx:125', e instanceof Error ? e.message : String(e)); }
    // (0) 基岩色：默认 Globe 无瓦片处为 BLACK（纯黑），在深空蓝色 Starfield 上就是"黑洞"。
    //     改成带一点点深蓝的基岩色 —— 瓦片没加载完的地方看起来是
    //     一致的真实深色海洋，而不是"破洞"。
    try { (viewer.scene.globe as unknown as { baseColor?: Cesium.Color }).baseColor = Cesium.Color.fromBytes(6, 14, 30, 255); } catch (e) { console.warn('[EmptyCatch] cesium/CesiumCanvas.tsx:129', e instanceof Error ? e.message : String(e)); }
    ...
    } catch (e) { console.warn('[EmptyCatch] cesium/CesiumCanvas.tsx:134', e instanceof Error ? e.message : String(e)); }
    // (0c) 隐藏瓦片加载失败的红色错误纹理 + 失败提示
    try { (viewer.scene.globe as unknown as { showTileFailures?: boolean }).showTileFailures = false; } catch (e) { console.warn('[EmptyCatch] cesium/CesiumCanvas.tsx:136', e instanceof Error ? e.message : String(e)); }
    try { (viewer.scene as unknown as { tileLoadFailureMessage?: boolean }).tileLoadFailureMessage = false; } catch (e) { console.warn('[EmptyCatch] cesium/CesiumCanvas.tsx:137', e instanceof Error ? e.message : String(e)); }
    ...
    try {
      ...
      try { (fbLayer as unknown as { _label?: string })._label = '__fallbackOceanNoise__'; } catch (e) { console.warn('[EmptyCatch] cesium/CesiumCanvas.tsx:195', e instanceof Error ? e.message : String(e)); }
      fbLayer.alpha = 1;
    ...
    try { (viewer.scene.globe as unknown as { showTileBoundaries?: boolean }).showTileBoundaries = false; } catch (e) { console.warn('[EmptyCatch] cesium/CesiumCanvas.tsx:203', e instanceof Error ? e.message : String(e)); }
    ...
    } catch (e) { console.warn('[EmptyCatch] cesium/CesiumCanvas.tsx:208', e instanceof Error ? e.message : String(e)); }
    ...
    } catch (e) { console.warn('[EmptyCatch] cesium/CesiumCanvas.tsx:220', e instanceof Error ? e.message : String(e)); }
    ...
    } catch (e) { console.warn('[EmptyCatch] cesium/CesiumCanvas.tsx:234', e instanceof Error ? e.message : String(e)); }
    ...
    } catch (e) { console.warn('[EmptyCatch] cesium/CesiumCanvas.tsx:256', e instanceof Error ? e.message : String(e)); }
    // (5) Cesium 原生 SkyBox：直接置 null（比 show=false 更干净，避免哪怕 show=false 的对象占资源）
    try { (viewer.scene as unknown as { skyBox?: Cesium.SkyBox | null }).skyBox = null; } catch (e) { console.warn('[EmptyCatch] cesium/CesiumCanvas.tsx:258', e instanceof Error ? e.message : String(e)); }
    ...
    try { (viewer.scene as unknown as { sun?: unknown }).sun = null; } catch {
      try { const s = (viewer.scene as unknown as { sun?: { show?: boolean } }).sun; s && (s.show = false); } catch (e) { console.warn('[EmptyCatch] cesium/CesiumCanvas.tsx:266', e instanceof Error ? e.message : String(e)); }
    }
    try { (viewer.scene as unknown as { moon?: unknown }).moon = null; } catch {
      try { const m = (viewer.scene as unknown as { moon?: { show?: boolean } }).moon; m && (m.show = false); } catch (e) { console.warn('[EmptyCatch] cesium/CesiumCanvas.tsx:269', e instanceof Error ? e.message : String(e)); }
    }
    ...
    try {
      ...
      for (let i = pc.length - 1; i >= 0; i--) {
        try { pc.remove(pc.get(i)); } catch (e) { console.warn('[EmptyCatch] cesium/CesiumCanvas.tsx:280', e instanceof Error ? e.message : String(e)); }
      }
    } catch (e) { console.warn('[EmptyCatch] cesium/CesiumCanvas.tsx:282', e instanceof Error ? e.message : String(e)); }
    ...
    } catch (e) { console.warn('[EmptyCatch] cesium/CesiumCanvas.tsx:300', e instanceof Error ? e.message : String(e)); }
    // (3) Globe 光照细节：normal-based shading（让地形在任何底图下都更有"皮纹"质感）
    try {
      (viewer.scene.globe as unknown as { showSkirts?: boolean }).showSkirts = true;
    } catch (e) { console.warn('[EmptyCatch] cesium/CesiumCanvas.tsx:304', e instanceof Error ? e.message : String(e)); }
    ...
    if (!ionToken) {
      ...
      void layerMgr.schedule('terrain', async () => {
        ...
        } catch (e) { console.warn('[EmptyCatch] cesium/CesiumCanvas.tsx:345', e instanceof Error ? e.message : String(e)); }
    ...
    if (amapKey) {
      ...
      const switchToAmap = () => {
        ...
        amapSwitched = true;
        try { offAmap(); } catch (e) { console.warn('[EmptyCatch] cesium/CesiumCanvas.tsx:388', e instanceof Error ? e.message : String(e)); }
        void Promise.resolve().then(async () => {
          if (cancelled) return;
          try {
            await controller.setBasemap('amapSatellite');
            useGeographyStore.getState().setBasemap('amapSatellite');
          } catch (e) { console.warn('[EmptyCatch] cesium/CesiumCanvas.tsx:394', e instanceof Error ? e.message : String(e)); }
        });
      };
    ...
    const applyDegrade = (cfg: DegradeConfig): void => {
      try {
        ...
        try {
          ...
          if (typeof sceneAny.highDynamicRange === 'boolean') sceneAny.highDynamicRange = false;
          if (typeof sceneAny.tonemapped === 'boolean') sceneAny.tonemapped = false;
        } catch (e) { console.warn('[EmptyCatch] cesium/CesiumCanvas.tsx:431', e instanceof Error ? e.message : String(e)); }
        ...
        try {
          const st = useGeographyStore.getState();
          const lessonActive = !!st.lesson?.activeLessonId || (window as unknown as { _lessonActiveForExaggerationGuard?: boolean })._lessonActiveForExaggerationGuard;
        ...
        } catch (e) { console.warn('[EmptyCatch] cesium/CesiumCanvas.tsx:475', e instanceof Error ? e.message : String(e)); }
        ...
        } catch (e) { console.warn('[EmptyCatch] cesium/CesiumCanvas.tsx:486', e instanceof Error ? e.message : String(e)); }
        ...
        } catch (e) { console.warn('[EmptyCatch] cesium/CesiumCanvas.tsx:493', e instanceof Error ? e.message : String(e)); }
        ...
        try { viewer.scene.sun && (viewer.scene.sun.show = false); } catch (e) { console.warn('[EmptyCatch] cesium/CesiumCanvas.tsx:497', e instanceof Error ? e.message : String(e)); }
        try { viewer.scene.moon && (viewer.scene.moon.show = false); } catch (e) { console.warn('[EmptyCatch] cesium/CesiumCanvas.tsx:498', e instanceof Error ? e.message : String(e)); }
    ...
    (() => {
      ...
      const finalize = () => {
        ...
        try { off(); } catch (e) { console.warn('[EmptyCatch] cesium/CesiumCanvas.tsx:590', e instanceof Error ? e.message : String(e)); }
        try { window.clearTimeout(tOut); } catch (e) { console.warn('[EmptyCatch] cesium/CesiumCanvas.tsx:591', e instanceof Error ? e.message : String(e)); }
        pushStartup(100, null);
      };
    ...
    const onGlobalError = (ev: ErrorEvent) => {
      ...
      if (typeof msg === 'string' && (msg.includes('Cesium') || msg.includes('tile') || msg.includes('render') || msg.includes('WebGL'))) {
        ...
        try { ev.preventDefault?.(); } catch (e) { console.warn('[EmptyCatch] cesium/CesiumCanvas.tsx:607', e instanceof Error ? e.message : String(e)); }
        try { viewer.scene.requestRender(); } catch (e) { console.warn('[EmptyCatch] cesium/CesiumCanvas.tsx:608', e instanceof Error ? e.message : String(e)); }
      }
    };
    const onUnhandledRejection = (ev: PromiseRejectionEvent) => {
      ...
      if (rStr && /Cesium|tile|ImageryProvider|Terrain|network|fetch|CORS|404|429|5\d\d/i.test(rStr)) {
        ...
        try { ev.preventDefault?.(); } catch (e) { console.warn('[EmptyCatch] cesium/CesiumCanvas.tsx:617', e instanceof Error ? e.message : String(e)); }
      }
    };
